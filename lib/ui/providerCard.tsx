...
import { Seller } from "../constant/type/product.type";
  ...
  if (viewMode === "list") {
    return (
      <Link
        ...
        className="group flex flex-col justify-between gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm transition-all duration-200 hover:border-blue-500/30 hover:shadow-md sm:flex-row sm:items-center"
      >
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500">
              <span className="flex items-center gap-1">
                ...
                {provider.businessLocation || provider.location || "N/A"}
              </span>
  ...
  return (
    ...
    >
      ...
      <div className="relative h-28 w-full bg-slate-100">
        ...
        <span
          className={`absolute right-3 top-3 rounded-full bg-white px-2.5 py-0.5 text-[11px] font-semibold capitalize shadow-sm ${
            isVerified
              ? "text-emerald-600"
              : provider.verificationStatus === "pending"
                ? "text-amber-600"
      ...
      <div className="relative flex flex-1 flex-col p-5">
        {/* Logo */}
        <div className="-mt-12 relative mb-3 h-16 w-16 overflow-hidden rounded-2xl border-2 border-white bg-white shadow-md">
          <Image
            src={provider.logo || "/placeholder-logo.png"}
            alt={displayName}
            fill
